...
class DispatchLog(TimestampMixin, Base):
    ...
    dispatch_id: Mapped[int] = mapped_column(primary_key=True)
    ...
    template_id: Mapped[int] = mapped_column(ForeignKey("notification_templates.template_id"), nullable=False)
    ...
    opened_timestamp: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    failure_reason: Mapped[str | None] = mapped_column(String(500))

    id = synonym("dispatch_id")
    user_id = synonym("agent_id")
    opened_at = synonym("opened_timestamp")
